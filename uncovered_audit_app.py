...
CST_SHIPPERS = ['Anheuser-Busch InBev Deutschland GmbH & Co KG','ARTSANA S.P.A.','Beiersdorf Customer Supply GmbH','BDSK Handels GmbH & Co.KG','Charles Kendall Freight','EST contracts B.V.','Coyote Logistics UK Ltd','DANONE NUTRICIA SPA','Danone UK','DANONE IT SN','Danone UK SN','DELONGHI APPLIANCES','Howard Tenens','DGL- Ingram (KSP) ES','Eddie Stobart (Appleton Culina House- Culina Group) Unilever','Great Bear Distribution CHORLEY','Great Bear Distribution Ltd (Spectrum)','Great Bear Distribution MV1','Great Bear Distribution OLDHAM','Great Bear MV1 - Helen of Troy','Great Bear Distribution SHEFFIELD','Great Bear Port Salford','Great Bear Port Salford Mars','H.J. Heinz BV','Hoover Ltd','iRobot UK Ltd','JACOBS DOUWE EGBERTS GB LTD','JYSK SE','Kellogg Marketing And Sales Company (UK) Limited','Keter Italia S.p.A.','Keter Iberia S.L.U','Keter Germany Gmbh','Keter France Sas','Mars PF France','Mars GmbH (CBT-DE)','Mars Multisales Spain S.L.','Mars Food Europe CV','Mars GmbH FLOERSHEIM','Mars GmbH MINDEN','Messaggerie Libri Spa','Moemax Logistik GmbH','Mondi Logistik GmbH','Nestle Enterprises SA, Business Growth Solutions Division','Nestle UK','Nestrade S.A. T-Hub Central','Nestrade SA (t-hub North)','Nestrade S.A. T-Hub North','Nestrade S.A. T-Hub South','Nestrade SA','Nestrade SA (Worms)','Nestrade T-hub West','PepsiCo Deutschland GmbH','Philips Consumer Lifestyle BV','Pregis Ltd','Procter & Gamble International Operations SA','Robert Bosch Power Tools GmbH','Samsonite GmBH','saturn petcare gmbh','Skechers EDC','SharkNinja Europe Ltd','SharkNinja Germany Gmbh','SIG Combibloc GmbH - Linnich','SIG Combibloc GmbH - Wittenberg','S.L. Systemlogistik GmbH','Soffass spa','Sofidel Germany GmbH','Sofidel France SAS','Sofidel Spain','Sofidel UK','Spectrum Brands (UK) Ltd','Tetra GmbH','TYRE ECO CHAIN','The Book Service Limited','Unilever Europe B.V. (UK)','Unilever Europe B.V. (DE)','Unilever Europe B.V. (EU)','Versuni Nederland B.V','Walkers Snacks Distribution Ltd','Wincanton (J SAINSBURY PLC)','Yankee Candle Co (Europe) LTD','Yankee Candle Co - DE','Zalando SE','Zeitfracht Medien GmbH','Danone Deutschland GmbH','Danone UK Waters','Danone FR','Tchibo GmbH','Hachette UK Distribution','Wacker Chemie AG','Electrolux Hausgeraete GmbH','Sharp Consumer Electronics Poland sp. z o.o.','Brita France','BRITA SE - Shipments Beselich','Brita Italia s.r.l. Unipersonale','home24 eLogistics GmbH & Co. KG','Cargill Poland Sp. z o.o.','Nitto Advanced Film Gronau GmbH','Cargill S.L.U.','Coca-Cola Europacific Partners Deutschland GmbH','Schlaadt HighCut GmbH','Fressnapf Logistics Management GmbH','tegut... gute Lebensmittel GmbH & Co. KG','Bio Springer','La Palette Rouge Iberica s.a. succ.le in Italia','COLGATE PALMOLIVE EUROPE','Hisense UK','ECOSCOOTING DELIVERY SL','EDT BE SRL (TEMU)','Eddie Stobart (Appleton Culina House- Culina Group)','L\'Oreal Italy','Geodis D&E Normandie','Hager Electro SAS','Heineken Deutschland GmbH','HarperCollins Publishers Ltd','Euro Pool System UK Ltd','3M EMEA GmbH','Falken Tyre Europe GmbH','SACHSENMILCH Leppersdorf GmbH','XPO Transport Solutions UK Limited','La Palette Rouge Iberica Sa','LPR - LA PALETTE ROUGE (GB) LTD','BONDUELLE RE','HARIBO Sp. z o.o.','Groupe SEB WMF Consumer GmbH','HOYER GmbH Internationale Fachspedition','BLACK & DECKER LIMITED BV','Cycleon B.v.','Wella International Operations Switzerland Sarl']
...
if st.session_state.step == 1:
    ...
    st.info('Removing: test orders, orders outside the current year, and orders created more than 2 months ago.')
    df = st.session_state.df_raw.copy()
    ic = len(df)
    log = []
    cm = {col.strip().lower(): col for col in df.columns}
    sc = cm.get('shipper')
    cc = cm.get('creation date and time')
    if sc:
        b = len(df)
        df = df[~df[sc].str.contains('test', case=False, na=False)]
        log.append('Removed {} test order(s).'.format(b - len(df)))
    if cc:
        df[cc] = pd.to_datetime(df[cc], errors='coerce', dayfirst=False)
        cy = datetime.now().year
        tma = datetime.now() - timedelta(days=60)
        b = len(df)
        df = df[(df[cc].dt.year == cy) & (df[cc] >= tma)]
        log.append('Removed {} order(s) outside current year or older than 2 months.'.format(b - len(df)))
