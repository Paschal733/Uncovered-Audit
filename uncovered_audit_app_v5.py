...
defaults = {
    ...
    'non_cst_final': None,
    'unmatched_count': 0,
    'step4_skipped': False,       # NEW: notification flag
    'batch_index': 0              # NEW: which batch user is viewing in Step 5
}
...
# ---- Step 4 ----
    ...
    # NEW: auto-skip if no external orders
    if ext.empty:
...
# ---- Step 5 ----
    ...
    # NEW: Batch copy (50 per batch), NO SERIAL NUMBERS in copy block
    batch_size = 50
    total = len(rids)
    ...
    with st.expander('View / Copy Order IDs (copy-paste into Unified Portal)'):
        if total == 0:
            ...
            # If more than 6 batches, show remaining as a selectbox
            if batch_count > 6:
                remainder = list(range(6, batch_count))
                sel = st.selectbox(
                    "More batches",
                    options=remainder,
                    format_func=lambda x: f"Batch {x+1}",
                    index=max(0, min(st.session_state.batch_index - 6, len(remainder) - 1))
                )
                if st.button("Go", key="go_more_batches"):
                    st.session_state.batch_index = sel
...
# ---- Step 6 ----
    ...
    # NEW: For CST FINAL results, remove Destination Stop Facility Name
    ...
    # NEW: CST download sheet also uses cf_clean (without Destination Stop Facility Name)
    fb = to_excel_bytes({
        'CST Orders': cf_clean if not cf_clean.empty else pd.DataFrame(columns=REQUIRED_COLUMNS_CST),
