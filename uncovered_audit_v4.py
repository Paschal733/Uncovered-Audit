...
def load_smc_file(f):
    raw = f.read()
    # Some SMC exports look like tab-separated with Sheet header
...
defaults = {
    ...
    'cst_ext': None,
    'non_cst_ext': None,
    'portal_ids': [],   # NEW: persists pasted/uploaded portal IDs so the Run button behaves correctly
    'cst_final': None,
    'non_cst_final': None,
    'unmatched_count': 0
}
...
# ---- Step 4 ----
    ...
    # NEW: guardrails to prevent crashes with unexpected export schemas
    ...
    if not ext.empty:
        ...
        # CST external orders remove Destination Stop Facility Name column
    ...
    st.warning(
        "ACTION REQUIRED\n"
        "1. Copy CST External Orders above to the CST Task Sheet (Uncovered tab).\n"
        "2. Copy Non-CST External Orders above to the AF Scheduling Daily Task Workbook (Uncovered tab).\n"
    ...
    with c2:
        if st.button('Done - Proceed to Step 5', type='primary'):
            ...
            # reset portal ids when moving into step 5 for a fresh entry
...
# ---- Step 5 ----
    ...
    # NEW: show IDs in a single column, numbered, one ID per row, copy-friendly
    with st.expander('View / Copy Order IDs (copy-paste into Unified Portal)'):
        numbered_lines = [f"{i+1}. {oid}" for i, oid in enumerate(rids)]
        ids_block = "\n".join(numbered_lines)
        st.text_area(
            "Order IDs (copy these)",
            value=ids_block,
            height=260,
            help="Each Order ID is on a new line and numbered serially."
        )

    st.divider()
    st.subheader('Unified Portal Instructions')
    st.markdown(
        "1. Copy the Order IDs from the panel above.\n"
    ...
    if im == 'Paste IDs directly':
        ...
        if pasted.strip():
            ...
            st.session_state.portal_ids = list(dict.fromkeys(ri))  # unique, keep order
    ...
    # NEW: button always renders, enabled once IDs exist (fixes "button not showing" problem)
...
# ---- Step 6 ----
    ...
    fb = to_excel_bytes({
        'CST Orders': cf if not cf.empty else pd.DataFrame(columns=REQUIRED_COLUMNS_CST),
    ...
    if st.button('Start a New Audit', type='primary'):
        # Safer reset: clear only our keys (avoid breaking widget internals)
        keys_to_clear = list(defaults.keys()) + ['smc_upload', 'portal_upload']
        for k in keys_to_clear:
            if k in st.session_state:
                del st.session_state[k]

        # re-init required defaults
        for k, v in defaults.items():
            if k not in st.session_state:
                st.session_state[k] = v
